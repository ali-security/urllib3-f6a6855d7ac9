...
if not hasattr(gzip, "compress"):
    # Platform-specific: Python 2.7 has no `gzip.compress`.
...
class TestResponse(object):
    ...
    # `br` is deliberately excluded from the strict bound below: Google's
    # brotli library does not fully respect the output buffer limit
    ...
    @pytest.mark.parametrize(
        "read_surface", ("read", "stream", "chunked_stream", "read_chunked")
    )
    @pytest.mark.parametrize(
        "data",
    ...
    def test_decompression_bomb_is_bounded(self, read_surface, data):
        """
        A highly compressible body must never be decoded in units larger
        than the requested ``amt``, on any read surface.

        Before the fix a single ``read(amt)``/``stream(amt)``/
        ``read_chunked(amt)`` call read ``amt`` *compressed* bytes and
        decompressed them without a limit, so a tiny body expanded to an
        unbounded amount of memory. This asserts the boundedness directly,
        without relying on a memory profiler.
        """
        name, compress_func = data
        amt = 64 * 1024
        original_data = b"\0" * (8 * 2 ** 20)  # 8 MiB
        compressed_data = compress_func(original_data)
        # Sanity check: the whole bomb fits in a single raw read of `amt`.
        assert len(compressed_data) < amt
        # A correct implementation needs exactly this many chunks; the cap
        # keeps a regression from looping forever instead of failing.
        max_chunks = 2 * (len(original_data) // amt + 1)

        if read_surface in ("chunked_stream", "read_chunked"):
            httplib_r = httplib.HTTPResponse(MockSock)
            httplib_r.fp = MockChunkedEncodingResponse([compressed_data])
            r = HTTPResponse(
                httplib_r,
            ...
            if read_surface == "chunked_stream":
                reader = r.stream(amt, decode_content=True)
            else:
                reader = r.read_chunked(amt, decode_content=True)
        else:
            fp = BytesIO(compressed_data)
            r = HTTPResponse(
                fp, headers={"content-encoding": name}, preload_content=False
            )
            if read_surface == "stream":
                reader = r.stream(amt, decode_content=True)
            else:
                reader = iter(lambda: r.read(amt, decode_content=True), b"")

        chunks = []
        for chunk in reader:
            chunks.append(chunk)
            assert len(chunks) <= max_chunks, "read surface did not terminate"

        # No chunk handed back may exceed the requested amount, ...
        assert chunks
        assert max(len(chunk) for chunk in chunks) <= amt
        # ... the internal buffer must not have retained the whole body, ...
        assert len(r._decoded_buffer) <= amt
        # ... and the body must still round-trip in full.
